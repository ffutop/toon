...
- **混合结构赛道**：具有嵌套或部分一致结构的数据集（TOON 对比 JSON、YAML、XML）。CSV 被排除在外，因为它无法正确表示这些结构。
- **纯扁平结构赛道**：具有扁平表格结构、适合使用 CSV 的数据集（CSV 对比 TOON、JSON、YAML、XML）。

## 检索准确率

<!-- automd:file src="../../benchmarks/results/retrieval-accuracy.md" -->

基准测试包含 209 个数据检索问题，用于评估大语言模型对不同输入格式的理解能力；测试选用了 4 个大语言模型。
...
| 数组被截断：末尾移除 3 行 | 17 | 字段一致 | ✓ | 100% |
| 超出声明长度的多余行 | 23 | 字段一致 | ✓ | 100% |
| 字段数量不一致<br>（第 10 行缺少 `salary` 字段） | 20 | 字段一致 | ✓ | 100% |
| 缺少必需字段<br>（多行缺少 `email` 字段） | 20 | 字段一致 | ✓ | 100% |
...
**表格化适用率：** 符合 TOON 表格化格式条件（由基本类型值构成的字段一致对象）的数组所占百分比
...
**关于 CSV 的说明：** 由于它仅支持 209 个问题中的 109 个（仅限扁平表格数据），因此未列入排名。虽然 CSV 对于简单表格数据而言 Token 使用效率极高，但它无法表示其他格式所能处理的嵌套结构。

#### 各模型准确率

4 个大语言模型在 209 个数据检索问题上的准确率：
...
本基准测试衡量的是**大语言模型对不同输入格式的理解能力和数据检索准确率**。每个模型都会接收格式化数据，并需要根据这些数据回答问题。它**不是**在测试模型生成 TOON 输出的能力，而是测试模型读取和理解 TOON 的能力。
...
5. **事件日志**（75 条日志）：部分一致数据，约 50% 为扁平日志，约 50% 带有嵌套的错误对象。
6. **嵌套配置**（1 份配置）：深度嵌套配置，表格化适用率极低。

**结构校验数据集：**

7. **对照组**：有效的完整数据集（作为校验基线）
8. **截断**：末尾移除 3 行的数组（测试 `[N]` 长度检测能力）
9. **多余行**：超出声明长度、多出 3 行的数组
10. **宽度不匹配**：字段数量不一致（第 10 行缺少 `salary` 字段）
11. **缺失字段**：系统性的字段缺失（多行缺少 `email` 字段）

#### 问题类型

209 个问题分为五大类动态生成：

- **字段检索（33%）**：可直接查到的值，或可以直接从记录中读取的值（包括布尔值和简单计数，如数组长度）
  - 示例：“Alice 的薪资是多少？” → `75000`
  - 示例：“订单 ORD-0042 中有多少件商品？” → `3`
  - 示例：“订单 ORD-0042 的客户姓名是什么？” → `John Doe`

- **聚合运算（30%）**：数据集层面的总数和平均值，以及单条件筛选（总数、求和、最小/最大值比较）
  - 示例：“有多少员工在工程部门工作？” → `17`
  - 示例：“所有订单的总收入是多少？” → `45123.50`
  - 示例：“有多少员工的薪资 > 80000？” → `23`

- **筛选过滤（23%）**：需要复合逻辑的多条件查询（跨字段的 AND 约束）
  - 示例：“销售部门中有多少员工的薪资 > 80000？” → `5`
  - 示例：“有多少在职员工的工作年限超过 10 年？” → `8`

- **结构感知（12%）**：测试模型能否利用输入格式自带的结构信息（如 TOON 的 `[N]` 计数和 `{fields}`，以及 CSV 的表头行）
  - 示例：“数据集中有多少员工？” → `100`
  - 示例：“列出员工记录的字段名” → `id, name, email, department, salary, yearsExperience, active`
  - 示例：“最后一名员工所在部门是什么？” → `Sales`

- **结构校验（2%）**：测试模型能否利用结构元数据识别不完整、被截断或损坏的数据
...
2. **向模型提问**：每个模型在提示词中接收格式化数据和问题，并提取答案。
3. **确定性校验**：使用类型感知的比较方式对答案进行校验（例如 `50000` = `$50,000`、`Engineering` = `engineering`、`2025-01-01` = `January 1, 2025`），无需借助大语言模型裁判。

#### 模型与配置

- **测试模型**：`claude-haiku-4-5-20251001`、`gemini-3-flash-preview`、`gpt-5-nano`、`grok-4-1-fast-non-reasoning`
- **Token 计数**：使用 `gpt-tokenizer` 和 `o200k_base` 编码（GPT-5 分词器）
- **温度参数**：未设置（使用模型默认值）
- **评测总数**：209 个问题 × 6 种格式 × 4 个模型 = 5,016 次大语言模型调用
