...
hero:
  name: TOON
  text: 面向 Token 的对象表示法
  tagline: 一种为 LLM 提示词设计的、紧凑且人类可读的 JSON 数据模型编码方式。
  image:
...
features:
  - title: 节省 Token 且准确率高
    icon: 📊
    details: 在跨 4 个模型的混合结构基准测试中，TOON 的准确率达到 76.4%(JSON 为 75.0%),同时所用 token 减少约 40%。
    link: /guide/benchmarks
  - title: JSON 数据模型
    icon: 🔁
  ...
  - title: 多语言生态
    icon: 🌐
    details: 提供由规范驱动的 TypeScript、Python、Go、Rust、.NET 等多种语言实现。
