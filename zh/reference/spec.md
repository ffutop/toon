...
**规范 v{{ $spec.version }}**（2026-05-20）是当前已发布的工作草案。该版本已足够稳定并可作为实现依据，但尚未定稿；详情请参阅规范中的 “Status of This Document” 部分。
...
定义了 TOON 这种面向行、基于缩进的表示法，并说明如何判断根值是对象、数组还是基本类型。

[§6 首部语法](https://github.com/toon-format/spec/blob/main/SPEC.md#6-header-syntax-normative)：
给出数组首部的规范性 ABNF 语法：`key[N<delim?>]{fields}:`。该节规定了方括号片段、分隔符符号和字段列表。
...
涵盖所有数组形式：基本类型数组（内联）、对象数组（表格化）、混合数组/非一致数组（列表），以及数组的数组。该节还包括表格化检测要求。
...
### 编码器检查清单（§13.1） [↗ SPEC.md](https://github.com/toon-format/spec/blob/main/SPEC.md#131-encoder-conformance-checklist)
...
### 解码器检查清单（§13.2） [↗ SPEC.md](https://github.com/toon-format/spec/blob/main/SPEC.md#132-decoder-conformance-checklist)

关键要求：

* 按 §6 解析数组首部（长度、分隔符、字段）
* 仅使用生效分隔符拆分内联数组和表格化行
...
### 校验器检查清单（§13.3） [↗ SPEC.md](https://github.com/toon-format/spec/blob/main/SPEC.md#133-validator-conformance-checklist)
